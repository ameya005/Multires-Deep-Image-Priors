...
class MultiresUNet(nn.Module):
    '''
        Multi-resolution UNet. Adds additional layers at the decoder
        levels, to reconstruct images at different sizes.
        upsample_mode in ['deconv', 'nearest', 'bilinear']
        pad in ['zero', 'replication', 'none']
    '''
    def __init__(self, num_input_channels=3, num_output_channels=3,  nlevels=3, 
                       feature_scale=4, more_layers=0, concat_x=False,
                       upsample_mode='deconv', pad='zero', norm_layer=nn.InstanceNorm2d, need_sigmoid=True, need_bias=True):
        super().__init__()
        ...
        self.up2 = unetUp(filters[1], upsample_mode, need_bias, pad)
        self.up1 = unetUp(filters[0], upsample_mode, need_bias, pad)
        self.out_conv3 = nn.Conv2d(filters[2], 3, kernel_size=1)
        self.out_conv2 = nn.Conv2d(filters[1], 3, kernel_size=1)
        self.final = conv(filters[0], num_output_channels, 1, bias=need_bias, pad=pad)
    ...
    def forward(self, inputs):
        ...
        up4= self.up4(up_, down3)
        up3= self.up3(up4, down2)
        out2 = self.out_conv3(up3)
        up2= self.up2(up3, down1)
        out1 = self.out_conv2(up2) 
        up1= self.up1(up2, in64)
        #print(up1.size(), up2.size(), up3.size(), up4.size(), out1.size(), out2.size())

        return  self.final(up1), F.sigmoid(out1), F.sigmoid(out2)
